...
function copyToClipboard(txt, showTxt = true) {
    ...
    document.body.removeChild(tmpInput);
    showTxt
        ? userLog('info', `${txt} 已复制到剪贴板 👍`, 'top-end')
        : userLog('info', `已复制到剪贴板 👍`, 'top-end');
}
...
function shareRoomByEmail() {
    Swal.fire({
        ...
        preConfirm: () => {
            ...
            if (!selectedDateTime) {
                Swal.showValidationMessage('请选择会议日期和时间');
                return false;
            }
            ...
            const roomPassword =
                isRoomLocked && (room_password || rc.RoomPassword)
                    ? '会议密码：' + (room_password || rc.RoomPassword) + newLine
                    : '';
            const emailSubject = `邀请您加入 ${BRAND.app.name} 视频会议`;
            const emailBody = `会议已安排在：${newLine}时间：${selectedDateTime}${newLine}${roomPassword}点击加入：${RoomURL}${newLine}`;
